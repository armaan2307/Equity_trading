...
# 120+ highly active, liquid constituents across Nifty 500 / Midcap / Smallcap
NIFTY_500_LIQUID = [
    ...
    "SUZLON", "IDEA", "YESBANK", "EXIDEIND", "AMBUJACEM", "ACC", "DALBHARAT", "JKCEMENT", "JSWDULUX", "SBFC"
]
...
def run_screener():
    ...
    # Batch process in lightweight chunks of 25 to prevent memory overflow
    chunk_size = 25
    chunks = [NIFTY_500_LIQUID[i:i + chunk_size] for i in range(0, len(NIFTY_500_LIQUID), chunk_size)]
    ...
    for chunk in chunks:
        ...
        for sym in chunk:
            try:
                df = data[f"{sym}.NS"] if (data is not None and f"{sym}.NS" in data) else None
                ...
                # 1. Intraday Momentum: Highest 1-day rate of change & volatility
                ...
                # 2. Swing: Trend continuation above EMA 20
                ...
                # 3. Longterm: Distance relative to SMA 20 baseline
    ...
    # Rank and extract the top 10 unique setups
